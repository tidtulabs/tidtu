...
export const getExamList = async (endpoint: string): Promise<GetExamListResult> => {
  ...
  rows.each((_, element) => {
    ...
    tds.each((index, tdElement) => {
      ...
      if (next.text().includes(">>")) {
        listResult.nextPagination = normalizeEndpoint(td.find("a").last().attr("href") || "");
      } else {
        listResult.nextPagination = null;
      }

      examList.isNew = td.find("img").last().attr("src")?.includes("news.gif") || false;
