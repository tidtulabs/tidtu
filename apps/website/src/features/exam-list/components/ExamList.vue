...
import { useVueTable, getCoreRowModel, getSortedRowModel, getFilteredRowModel, getExpandedRowModel } from '@tanstack/vue-table'
...
const {
  exams, isPending, isFetching, isError, error,
  fetchingFlag, downloadingRows, fetchMore, downloadFile,
} = useExamListData()

const {
  columnVisibility, showUploadDateOnMobile, showPageOnMobile, toggleUploadDate, togglePage,
...
const columns: ColumnDef<ExamItem>[] = [
  ...
  },
  {
    ...
    header: 'Số trang',
    cell: ({ row }) => h('div', { class: 'text-foreground text-sm' }, row.getValue('page') as string),
  },
  {
    accessorKey: 'uploadDate',
    header: 'Ngày tải lên',
    cell: ({ row }) => h('div', { class: 'capitalize text-foreground text-sm' }, row.getValue('uploadDate') as string),
  },
  {
    accessorKey: 'isNew',
  },
...
<template>
  ...
  >
    ...
    <QuickBugButton
      ...
      :context="{ message: 'Có lỗi xảy ra khi tải dữ liệu', page: currentUrl }"
    />
  </div>
