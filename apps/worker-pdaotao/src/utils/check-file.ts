...
const checkFileType = async (url: string) => {
  try {
    ...
    const fileType = Object.entries(contentTypeMap).find(([key]) => contentType.includes(key))?.[1];
...
const checkFileExtension = (url: string) => {
  ...
  const matchedFormat = supportedFormats.find(({ extension }) => url.endsWith(extension));
